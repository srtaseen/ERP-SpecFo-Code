...
function calcsizqty(event) {
//    var SelVal = $("[id*='drpPacktype'] :selected").val();
//    if (SelVal == "Ratio") {
//        document.getElementById("txtnoPack").removeAttribute('disabled', false);
//        document.getElementById("txtnoPack").value = "1";
//    }
//    else {
//        document.getElementById("txtnoPack").setAttribute('disabled', true);
//        document.getElementById("txtnoPack").value = "1";
//    }

    var gvDrv = document.getElementById("grdPackCountry");
    var blnc = $("[id$='txtBalanceqty']").val();
    if ((document.getElementById("txtnoPack").value).length == 0) {
        document.getElementById("txtnoPack").value = "1";
    }   
    var Rat = $("[id$='txtnoPack']").val();
    var gttl = "0";
    var rttl = "0";
    for (i = 1; i < gvDrv.rows.length - 1; i++) {
        var Loopcell = gvDrv.rows[i].cells;
    ...
    if (parseFloat(gttl) > parseFloat(blnc)) {
        var grdtl = "0";
        document.getElementById("txtBalanceqty").style.border = '1px solid red';
        gvDrv.rows[grdrow].cells[lstclm].style.border = '1px solid red';
        alert("Size Quantity Can't Exceed PO Quantity");
        var trg = event.target || event.srcElement;
        var cid = trg.id
        var cval = document.getElementById(cid).value="0";
        gttl = "0";
        document.getElementById("txtBalanceqty").style.border = '';
        gvDrv.rows[grdrow].cells[lstclm].style.border = '';
        //document.getElementById("txtnoPack").value = "1";
        for (i = 1; i < gvDrv.rows.length - 1; i++) {
            var Loopcell = gvDrv.rows[i].cells;
...
function rattype(txtrat, gridview, blnceqty) {
    var ddl = document.getElementById("drpPacktype");
